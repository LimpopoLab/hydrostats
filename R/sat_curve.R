...
sat_curve <- function(temp, unit = "C") {
      ...
      if (exists("t")) {
            ...
            for (i in 1:length(t)) {
                  est <- hydrostats::e_star(t[i]) # this returns e*, the saturation vapor pressure from hydrostats::e_star
                  t_r <- 1 - (373.15/t[i]) # internal reference for t_r, below
                  sat_curve[i] <- (373.15 * est / (t[i]^2)) * (13.3185 - (3.952 * t_r) - (1.9335 * (t_r^2)) - (0.5196 * (t_r^3)))
            }
            return(sat_curve)
      }
}